        ...
        public static bool Prefix(TaskPanelBehaviour __instance, [HarmonyArgument(0)] string str)
        {
            foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
            {
                ...
                if (divertPowerTask != null)
                {
                    ...
                    {
                        if (Utils.IsDivertPowerTaskConsole(console))
                        {
                            consoles++;
                        }
                    }
                    ...
                    {
                        str += "\n" + TranslationController.Instance.GetString(SystemTypes.Electrical) + ": Divert Power (0/" + consoles.ToString() + ")";
                    }
                    else if (divertPowerTask.TaskStep == 1)
                    {
                        str += "\n<color=#FFFF00FF>" + TranslationController.Instance.GetString(mapName) + ": Accept Diverted Power (" + (consoles - DivertPower.tasks[divertPowerTask.TargetSystem].Count).ToString() + "/" + consoles.ToString() + ")</color>";
                    }
                    else
                    {
                        str += "\n<color=#00DD00FF>" + TranslationController.Instance.GetString(mapName) + ": Divert Power (" + consoles.ToString() + "/" + consoles.ToString() + ")</color>";
                    }
                }
            }
